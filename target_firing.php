 ...
 */
function get_sum_damages($ships){
  $sum_damages = 0;
...
function get_best_ratio($ships){
  ...
  return array("best_i" => $i, "best_ratio" => $ratio);
}
...
for ($i = 0; $i < $N; $i++)
{
    ...
    $damage_per_turn = ($SHIP == TYPE_CRUISER ? DAMAGE_PER_TURN : DAMAGE_PER_TURN * 2);
    $max_damage_per_turn = $ARMOR >= $damage_per_turn ? 1 : $damage_per_turn - $ARMOR;
    $turns_to_die = ceil($HP / $max_damage_per_turn);
    $ships[$i] = array(
      "i" => $i,
